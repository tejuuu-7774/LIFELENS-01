...
export default function ViewJournal() {
  ...
  const analyzeEntry = async () => {
    setAnalyzing(true);
    try {
      const res = await api.post("/api/ai/analyze", { text: form.content });
  ...
  const cancelEdit = () => {
    ...
    setForm({
      ...
      entryDate: journal.entryDate.slice(0, 10),
    });
    setEditMode(false);
  };
  ...
  const renameTag = async () => {
    ...
    try {
      await api.put(`/api/tags/${form.category}`, {
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-4 py-2 bg-white border rounded-lg text-[#3E5A44]"
            >
              Back
            </button>
          ...
          {editMode ? (
            <div className="space-y-4">
              <input
                ...
                className="w-full text-xl p-4 rounded-lg border"
              />
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                >
                  ...
                  <option value="angry">anggry</option>
              ...
              <div className="flex gap-3 mt-4">
                <button
                  type="button"
                  onClick={saveChanges}
                  className="px-5 py-2 bg-[#2f6f56] text-white rounded-lg"
                >
                  Save
                </button>
            ...
            <>
              <h2 className="text-2xl md:text-3xl font-bold text-[#2f513f] mb-3">
                {journal.title}
              </h2>

              <div className="flex items-center gap-3 mb-6">
                {journal.category?.name && (
                  <span className="px-3 py-1 bg-[#E8F7EF] text-[#2f6549] rounded-full text-sm">
                    #{journal.category.name}
                  </span>
                )}

                <span className="px-3 py-1 bg-[#DDF4F7] text-[#21565f] rounded-full text-sm">
                  {journal.mood}
                </span>
              ...
              >
                {analyzing ? "Analyzing..." : "AI Reflection"}
              </button>
